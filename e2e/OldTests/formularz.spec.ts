...
test("should correctly fill out the form and show a confirmation modal after submission", async ({
  page,
}) => {
...
async function selectAndCheckDateOfBirth(
  page: Page,
  day: string,
  month: string,
  year: string
...
async function fillAndCheck_Subjects(
  page: Page,
  subjects: string[]
): Promise<void> {
...
async function selectAndCheck_Picture(
  page: Page,
  pathToFile: string,
  fileName: string
) {
  ...
  // check (fakepath?)
  await expect(page.getByLabel("Select picture")).toHaveValue(
    `C:\\fakepath\\${fileName}`
  );
}

async function selectAndCheck_StateAndCity(
  page: Page,
  state: string,
  city: string
) {
